                ...
                <div class="card-body  p-0" >
                    <div class="d-flex flex-wrap gap-2 mb-3">
                        <button type="button" class="btn btn-primary waves-effect waves-light"
                                data-bs-toggle="modal" data-bs-target="#create-new-category88">
                            <i class="bx bx-add-to-queue font-size-16 align-middle me-2"></i>أضافه الأدلة والشواهد
                        </button>
                    </div>
                    <table id="datatable" class="table table-bordered dt-responsive  nowrap w-100 table-striped">
                        ...
                        <tbody>
                        @forelse($invoices  as $output)
                            <tr>
                                <td>{{ $loop->iteration }}</td>
                                <td> <p>{{ $output->name }} </p> </td>
                                <td><a href="{{ asset(PUBLIC_PATH .$output->file_path) }}"><i class="fa fa-file-alt"></i> {{$output->file_path}} </a></td>
                                <td>
                                    <div class="btn-group">
                                        <!--
                                        <a href="javascript:void(0);" data-category-id="{{ $output->id }}"
                                           ...
                                           -->
                                        <form action="{{ route('dashboard.week_invoices.destroy', $output->id) }}"
