...
@push('title','المهارات العلمية والعمليه')
...
@section('content')
        ...
        <div class="col-md-9">
            <div class="card">
                <div class="card-header">
                    <div class="card-title">المهارات العلمية والعملية :: </div>
                </div>
                <div class="card-body">
                    ...
                    <div class="d-flex flex-wrap gap-2 mb-3">
                        <button type="button" class="btn btn-primary waves-effect waves-light"
                            ...
                            <i class="bx bx-add-to-queue font-size-16 align-middle me-2"></i>أضافه مهارات علمية وعمليه
                        </button>
                    </div>
                    <table id="datatable" class="table table-bordered dt-responsive  nowrap w-100 table-striped">
                        ...
                        <tbody>
                        @forelse($WorkSkills  as $WorkSkill)
                            <tr>
                                <td>{{ $loop->iteration }}</td>
                                <td> <p>{{ $WorkSkill->name }} </p> </td>
                                </td>
                                <td>
                                    <div class="btn-group">
                                        <a href="javascript:void(0);" data-category-id="{{ $WorkSkill->id }}"
                                           class="text-muted font-size-20 edit"><i class="bx bxs-edit"></i></a>
                                        <form action="{{ route('dashboard.workskills.destroy', $WorkSkill->id) }}"
    ...
    @include('admins.programs.details.workskills.modals.store-modal')
    @include('admins.programs.details.workskills.modals.edit-modal')
@endsection
...
@push('scripts')
    ...
    @include('admins.programs.details.workskills.scripts.store')
    @include('admins.programs.details.workskills.scripts.delete')
    @include('admins.programs.details.workskills.scripts.edit')
@endpush
