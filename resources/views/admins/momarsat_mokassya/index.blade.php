...
<div class="row">
                ...
                <div class="card-body">
                    <div class="d-flex flex-wrap gap-2 mb-3">
                        <button type="button" class="btn btn-primary waves-effect waves-light"
                            ...
                            <i class="bx bx-add-to-queue font-size-16 align-middle me-2"></i>أضافه ممارسة جديد
                        </button>
                    </div>
                    <div class="table-responsive">
                        <table id="datatable" class="table table-bordered dt-responsive  nowrap w-100 table-striped">
                            ...
                            <tbody>
                            @forelse($Momarsas  as $momarsa)
                                <tr>
                                    <td>{{ $loop->iteration }}</td>
                                    <td style="text-align: right"><a href="{{route('dashboard.momarsat_mokassy_files.show' , $momarsa->id )}}">{{ $momarsa->name }} </a> </td>
                                    <td>
                                        <div class="btn-group">
                                            <a href="javascript:void(0);" data-category-id="{{ $momarsa->id }}"
                                               class="text-muted font-size-20 edit"><i class="bx bxs-edit"></i></a>
                                            <form action="{{ route('dashboard.momarsat_mokassy.destroy', $momarsa->id) }}"
    ...
    @include('admins.momarsat_mokassya.modals.store-modal')
    @include('admins.momarsat_mokassya.modals.edit-modal')
@endsection
@push('scripts')
    ...
    @include('admins.momarsat_mokassya.scripts.store')
    @include('admins.momarsat_mokassya.scripts.delete')
    @include('admins.momarsat_mokassya.scripts.edit')
@endpush
